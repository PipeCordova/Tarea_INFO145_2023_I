...
int main(int argc, char* argv[]){
    ...
    if(FB){
        ...
        clock_t t1 = clock();
        int f1 = formasFB(n,k,potencias,rotos);
        clock_t t2 = clock();
    ...
    formaCorta = formaMasCortaPD(n,k,potencias,rotos);
    int largoFC = formaCorta.size();
    cout << "Largo del camino\t= " << largoFC - 1 << endl;
    cout << "Camino a seguir\t\t= ";
    if(formaCorta.empty()){
        cout << "No existe camino posible!" << endl;
    }
    else{
        for(int i = 1; i<largoFC; i++){
            cout << formaCorta[i] << " " ;
        }
        cout << endl;
    }
...
/*  Algoritmo de Fuerza Bruta que calcula la cantidad de formas posibles de subir la escalera de tamaño n,
...
/*  Funcón destinada a imprimir un vector   */
void imprimeVector(vector<int> v){
    if(!v.empty()){
